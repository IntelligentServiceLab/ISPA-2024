...
class Model(nn.Module):
    ...
    def calcLosses(self, ancs, poss, negs, adj):
        ...
        if args.ssl1_reg != 0 or args.ssl2_reg != 0:
            ...
            sslLoss1 = -(scores2.detach() * (scores1 + 1e-8).log() + (1 - scores2.detach()) * (
                    1 - scores1 + 1e-8).log()).mean() * args.ssl1_reg
            sslLoss2 = -(scores1.detach() * (scores2 + 1e-8).log() + (1 - scores1.detach()) * (
                    1 - scores2 + 1e-8).log()).mean() * args.ssl2_reg
